...
describe('posts route', () => {
	...
	it('returns the auth error response when POST is unauthorized', async () => {
		...
		const response = await POST(
			jsonRequest('http://localhost/api/posts', { title: 'x', body: 'y' }),
		);
	...
	it('rejects invalid POST payloads', async () => {
		...
		const badBody = await POST(
			jsonRequest('http://localhost/api/posts', { title: 'title', body: '' }),
		);
	...
	it('rejects a page number above maxPage', async () => {
		...
		const response = await GET(
			new Request('http://localhost/api/posts?page=2&limit=20'),
		);
