import { createPost, getPosts } from '@/lib/api/posts.client';
...
describe('posts client', () => {
  ...
  it('throws the server response text when createPost fails', async () => {
    ...
    await expect(createPost({ title: 'Hello', body: 'World' })).rejects.toThrow('boom');
  });

  it('fetches posts with default pagination values', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ posts: [], page: 1, maxPage: 1, total: 0, limit: 20 }),
    } as Response);
  ...
  it('adds from and to parameters when both are provided', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ posts: [], page: 2, maxPage: 3, total: 40, limit: 10 }),
    } as Response);
