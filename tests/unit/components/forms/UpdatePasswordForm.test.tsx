...
describe('UpdatePasswordForm', () => {
  ...
  it('submits the password change and resets the form on success', async () => {
    vi.mocked(authClient.changePassword).mockImplementation(async (_values: any, options: any) => {
      options.onSuccess();
    });
  ...
  it('does not submit when the new password matches the current password', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/^Current password/i), 'same-password');
    await userEvent.type(screen.getByLabelText(/^New password/i), 'same-password');
    await userEvent.type(screen.getByLabelText(/^Repeat password/i), 'same-password');
    await userEvent.click(screen.getByRole('button', { name: 'Update' }));
  ...
  it('sets a field error when the current password is invalid', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/^Repeat password/i), 'new-password-123');
    await userEvent.click(screen.getByRole('button', { name: 'Update' }));
